...
class SE_Conv_Block(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None, drop_out=False, size=(800, 800)):
        ...
        if planes <= 16:
            self.globalAvgPool = nn.AvgPool2d(size, stride=1)  # (224, 300) for ISIC2018
            self.globalMaxPool = nn.MaxPool2d(size, stride=1)
        elif planes == 32:
            self.globalAvgPool = nn.AvgPool2d((size[0]//2, size[1]//2), stride=1)  # (112, 150) for ISIC2018
            self.globalMaxPool = nn.MaxPool2d((size[0]//2, size[1]//2), stride=1)
        elif planes == 64:
            self.globalAvgPool = nn.AvgPool2d((size[0]//4, size[1]//4), stride=1)    # (56, 75) for ISIC2018
            self.globalMaxPool = nn.MaxPool2d((size[0]//4, size[1]//4), stride=1)
        elif planes == 128:
            self.globalAvgPool = nn.AvgPool2d((size[0]//8, size[1]//8), stride=1)    # (28, 37) for ISIC2018
            self.globalMaxPool = nn.MaxPool2d((size[0]//8, size[1]//8), stride=1)
        elif planes == 256:
            self.globalAvgPool = nn.AvgPool2d((size[0]//16, size[1]//16), stride=1)    # (14, 18) for ISIC2018
            self.globalMaxPool = nn.MaxPool2d((size[0]//16, size[1]//16), stride=1)
