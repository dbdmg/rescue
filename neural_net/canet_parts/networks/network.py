...
from neural_net.canet_parts.layers.modules import conv_block, UpCat, UpCatconv, UnetDsv3, UnetGridGatingSignal3
from neural_net.canet_parts.layers.grid_attention_layer import GridAttentionBlock2D, MultiAttentionBlock
from neural_net.canet_parts.layers.channel_attention_layer import SE_Conv_Block
from neural_net.canet_parts.layers.scale_attention_layer import scale_atten_convblock
from neural_net.canet_parts.layers.nonlocal_layer import NONLocalBlock2D
...
class Comprehensive_Atten_Unet(nn.Module):
    def __init__(self, in_ch=3, n_classes=1, feature_scale=4, is_deconv=True, is_batchnorm=True,
        ...
        self.up4 = SE_Conv_Block(filters[4], filters[3], drop_out=True, size=self.im_size)
        self.up3 = SE_Conv_Block(filters[3], filters[2], size=self.im_size)
        self.up2 = SE_Conv_Block(filters[2], filters[1], size=self.im_size)
        self.up1 = SE_Conv_Block(filters[1], filters[0], size=self.im_size)
    ...
    def forward(self, inputs):
        ...
        atten3_map = ndimage.interpolation.zoom(atten3_map, [1.0, 1.0, self.im_size[0] / atten3_map.shape[2], self.im_size[1] / atten3_map.shape[3]], order=0)
        ...
        atten2_map = ndimage.interpolation.zoom(atten2_map, [1.0, 1.0, self.im_size[0] / atten2_map.shape[2], self.im_size[1] / atten2_map.shape[3]], order=0)
        ...
        atten1_map = ndimage.interpolation.zoom(atten1_map, [1.0, 1.0, self.im_size[0] / atten1_map.shape[2], self.im_size[1] / atten1_map.shape[3]], order=0)
