...
class ResNet(nn.Module):
    def __init__(self, block, input_channels, layers=(3, 4, 23, 3)):
        """
        @param block: implementation of a single residual block
        @input_channels: 3 for RGB images (standard, also available with pretrained weights).
                     Use higher number if the images contain more channels
        @param layers: list of the number of blocks contained in each of the 4 layers
        """
        self.inplanes = 64
    ...
    def forward(self, x):
        # Run the first four layers of resnet
        x = self.conv1(x)
        x = self.bn1(x)
        x = self.relu(x)
        x = self.maxpool(x)
...
def resnet18(pretrained=True, input_channels=3):
    model = ResNet(BasicBlock, input_channels, [2, 2, 2, 2])
    if pretrained:
        load_weights_sequential(model, model_zoo.load_url(model_urls['resnet18']))
    return model


def resnet34(pretrained=True, input_channels=3):
    model = ResNet(BasicBlock, input_channels, [3, 4, 6, 3])
    if pretrained:
        load_weights_sequential(model, model_zoo.load_url(model_urls['resnet34']))
    return model


def resnet50(pretrained=True, input_channels=3):
    model = ResNet(Bottleneck, input_channels, [3, 4, 6, 3])
    if pretrained:
        load_weights_sequential(model, model_zoo.load_url(model_urls['resnet50']))
    return model


def resnet101(pretrained=True, input_channels=3):
    model = ResNet(Bottleneck, input_channels, [3, 4, 23, 3])
    if pretrained:
        load_weights_sequential(model, model_zoo.load_url(model_urls['resnet101']))
    return model


def resnet152(pretrained=True, input_channels=3):
    model = ResNet(Bottleneck, input_channels, [3, 8, 36, 3])
    if pretrained:
